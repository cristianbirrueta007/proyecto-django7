...
from .models import Alumnos, ComentarioContacto, Comentario
...
def consultar4(request):
     alumnos=Alumnos.objects.filter(turno__contrains="Vesp")
     return render(request,"registros/consultas.html",{'alumnos':alumnos})
...
def consultar7(request):
     alumnos=Alumnos.objects.filter(comentario_coment_contains="No inscrito")
     return render(request,"registros/consultas.html",{'alumnos':alumnos})

def consulta_fechas(request):
    fechaInicio = datetime.date(2025, 11, 20)
    fechaFin = datetime.date(2025, 11, 26)

    comentarios = ComentarioContacto.objects.filter(
        created_date_range=(fechaInicio, fechaFin)
    )
...
def consultar8(request):
    ...
    return render(request, "registros/con.html", {'comentarios': comentarios})

def consultar9(request):
     comentarios = ComentarioContacto.objects.filter(usuario="Pedro")
     return render(request,"registros/con.html",{'comentarios': comentarios})
...
def consultar11(request):
    comentarios = ComentarioContacto.objects.filter(usuario__endswith="a")
    return render(request, "registros/con.html", {'comentarios': comentarios})
